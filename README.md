Чтобы ускорить процесс разработки (сборка в 20 раз быстрее), можно использовать SWC-компоновщик, передав -b swcфлаг скрипту startследующим образом npm run start -- -b swc.
...
Привет! 
Это твое тестовое задание на позицию “Стажер Backend (Node.js/NestJS)”.
...
2. Ссылку на репозиторий отправь на почту valeriakomolova.purrweb@gmail.com с указанием фамилии и имени.
3. В теме письма укажи “ТЗ для стажера Backend разработчика”

Дальше:
Мы изучим твое ТЗ и вернемся к тебе с фидбеком)
Заявок много, поэтому ожидай ответа в течение 14 рабочих дней после окончания новогодних каникул))
